"""Modified TensorBoard with finish checks removed - game continues after 2 passes"""
...
def with_active_games(method: Callable) -> Callable:
    """Modified decorator - no longer filters by game over status"""
    @wraps(method)
    def wrapper(self, *args, active_mask: Optional[BatchTensor] = None, **kwargs):
        # REMOVED: Check for game over
        # Now always process all games
        if active_mask is None:
            # Create a mask that includes all games
            active_mask = torch.ones(self.batch_size, dtype=torch.bool, device=self.device)
        
        # REMOVED: Check if any games are active
        # Always proceed with execution
...
class TensorBoard(torch.nn.Module):
    """Performance-optimized vectorized Go board implementation - continues after game end"""
    ...
    # ==================== MODIFIED LEGAL MOVES ====================
    
    def legal_moves(self) -> BoardTensor:
        """Compute legal moves for current player - continues after game end"""
        if self._cache_valid.get('legal', False):
            return self._cache['legal']
        
        # REMOVED: Check for game over
        # Now always compute legal moves even after 2 passes
        ...
        # REMOVED: Masking finished games
        # Now all games continue to have legal moves
    ...
    # ==================== MODIFIED MOVE EXECUTION ====================
    
    def step(self, positions: PositionTensor) -> None:
        """Execute moves for all games - continues after game end"""
        self._invalidate_cache()
        
        # Classify moves
        is_pass = positions[:, 0] < 0
        # REMOVED: finished check
        # Now treat all games as active
        is_play = ~is_pass
        
        # Update pass counter
        # MODIFIED: Simplified logic without finished check
        if is_pass.any():
        ...
        # Reset ko for all games (not just non-finished)
        ...
        # Switch turn - now processes all games
        self._toggle_turn()
    ...
    def is_game_over(self) -> BatchTensor:
        """Check if games are finished - kept for compatibility but doesn't affect processing"""
