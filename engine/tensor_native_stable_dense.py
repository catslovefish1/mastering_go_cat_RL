...
class TensorBoard(torch.nn.Module):
    ...
    def _filter_super_ko_vectorized(self, legal_mask: BoardTensor, info: Dict) -> BoardTensor:
        ...
        # New: no broadcast temp; filter after nonzero
        b_all, i_all, j_all = cap_mask.nonzero(as_tuple=True)   # (Kcap_total,)
        keep = cand_cap[b_all, i_all]                           # (Kcap_total,) bool
        b_idx = b_all[keep]; i_idx = i_all[keep]; j_idx = j_all[keep]
        
        key = b_idx * N2 + i_idx                                   # (K,)
        # Sort once by key
        b_s, i_s, j_s =  b_idx, i_idx, j_idx
        
        # Segment boundaries per unique (b,i)
        uniq, counts = torch.unique_consecutive(key, return_counts=True)
