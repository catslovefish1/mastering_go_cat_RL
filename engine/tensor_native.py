...
class TensorBoard(torch.nn.Module):
    ...
    def legal_moves(self) -> BoardTensor:
        ...
        legal = empty & (liberty_counts > 0)
        
        # Rule 2: Can capture opponent groups
        legal |= self._check_capture_moves(empty)
        
        # Rule 3: Cannot play ko
        legal = self._apply_ko_rule(legal)
    ...
    def step(self, positions: PositionTensor) -> None:
        ...
        # Update hash for turn change
        self._update_hash_for_turn()
    ...
    def _update_hash_for_turn(self) -> None:
    ...
    def _update_hash_for_stones(
        ...
        for i, (b, c, idx) in enumerate(zip(batch_idx, colors, flat_idx)):
            hash_updates[b] = self.zobrist_position_keys[c].flatten()[idx]
        
        self.position_hash ^= hash_updates
    ...
    def _get_neighbor_opponent_groups(
        ...
        # Neighbor offsets
        offsets = torch.tensor([[0, 1], [0, -1], [1, 0], [-1, 0]], device=self.device)
        
        # Find all neighbor positions
        neighbor_mask = torch.zeros((B, H, W), dtype=torch.bool, device=self.device)
        
        for i, (b, r, c, color) in enumerate(zip(batch_idx, rows, cols, colors)):
            for dr, dc in offsets:
                nr, nc = r + dr, c + dc
                if 0 <= nr < H and 0 <= nc < W:
                    neighbor_mask[b, nr, nc] = True
        
        # Filter for opponent stones
        opponent_stones = self.get_opponent_stones()
        return neighbor_mask & opponent_stones
    ...
    def _remove_captured_stones(self, captured: BoardTensor) -> None:
        ...
        # Update hash
        for b, color, r, c in zip(batch_idx, opponent_color, rows, cols):
            self.position_hash[b] ^= self.zobrist_position_keys[color, r, c]
        ...
        for b in batch_idx.unique():
            ...
            if batch_captures == 1:
                ...
                self.ko_points[b] = torch.tensor([rows[idx], cols[idx]], dtype=torch.int16)
