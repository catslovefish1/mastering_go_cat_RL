"""tensor_board_elegant.py - Simplified and elegant Go engine.

Key improvements:
1. Eliminated duplicate hash update patterns
2. Unified active game masking
...
class TensorBoard(torch.nn.Module):
    ...
    def _process_captures(self, batch_idx: Tensor, rows: Tensor, cols: Tensor) -> None:
        ...
        if seeds.sum() == 0: return      # still a tensor
    ...
    def _flood_fill(self, seeds: BoardTensor, mask: BoardTensor) -> BoardTensor:
        ...
        while True:
            ...
            new_groups = groups | expanded
            if (new_groups == groups).all():   # tensor compare (no .item())
                return groups 
            groups = new_groups
        # for _ in range(self.board_size * self.board_size): 
        #     expanded = (self._count_neighbors(groups) > 0) & mask & ~groups
        #     groups |= expanded 
        # return groups    # loop becomes no-op once expansion stops
        # # while True:
        # #     expanded = (self._count_neighbors(groups) > 0) & mask & ~groups
        # #     if not expanded.any():
        # #         break
